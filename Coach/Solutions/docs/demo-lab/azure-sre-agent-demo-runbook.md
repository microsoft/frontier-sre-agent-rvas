...
| --- | --- | --- |
| Connectors | 3 data-plane (`github`, `github-mcp`, `microsoft-learn-mcp`) + 2 ARM telemetry (`log-analytics`, `application-insights`) | Outbound tools/data: GitHub for S2/S3, Learn for grounded answers, telemetry for the agent's own logs |
...
| Correlate an incident to source code and open a GitHub issue/PR | `github`/`github-mcp` connector + `code-analyzer` + `grubify` repo | S2 |
...
| S2 | Same Sev1 app incident, but root cause is in source code | Same alert as S1 | After the Azure triage, hands off to `code-analyzer`, which reads the Grubify repo over the GitHub connector, pinpoints the code, and opens an issue/PR carrying incident context | `github` / `github-mcp` connector + `grubify` repo (`authConnectorName`) + `code-analyzer` |
...
```bash
Student/Resources/scenarios/scripts/sre-agent-config.sh verify --target incident-filters \
...
**Preconditions — GitHub connection (decision required).** Code/issue/PR actions
need a GitHub connection. Two supported options:

| Option | What it is | Status in this lab | When to choose |
| --- | --- | --- | --- |
| **A — GitHub OAuth connector** | `connectors/github.yaml` (`dataConnectorType: GitHubOAuth`) + `repos/grubify.yaml` linked via `spec.authConnectorName`. | Applied, but requires a **one-time interactive OAuth authorization** in the SRE Agent portal before GitHub tools work. | Tightest portal integration; enables the SRE Agent repo deep-clone. |
| **B — GitHub MCP server connector** | `connectors/github-mcp.yaml` — GitHub's official remote MCP server (`dataConnectorType: Mcp`, `type: http`, endpoint `https://api.githubcopilot.com/mcp/`, `authType: BearerToken`) authenticated with a PAT injected via `${GITHUB_PAT}`. Default toolset (context, repos, issues, pull_requests, users) covers S2/S3. | **Implemented.** Manifest applied; needs a valid `GITHUB_PAT` injected at runtime. No interactive OAuth. | **Recommended for this demo:** PAT-scoped, non-interactive, scriptable. |

> **Action before demoing S2/S3:** choose one path. **Option B (recommended):** create
> a fine-grained PAT scoped to the Grubify repo (Contents: Read, Issues: Read/write,
> and Pull requests: Read/write for S2), export it as `GITHUB_PAT`, and apply the
> `github-mcp` connector (see §7.1). **Option A:** complete the GitHub OAuth
> authorization in the SRE Agent portal. Until one is done, S2/S3 will investigate
> but cannot create issues/PRs.

### 7.1 Enable Option B — GitHub MCP connector

The connector manifest is committed with a `${GITHUB_PAT}` placeholder; the PAT is
never stored in Git. Inject it in your shell and apply:

```bash
SUB=<Your Subscription ID>
RG=rg-sec-sreagent
AGENT=contoso-sre-agent-dev
# Option B.1 (recommended for the demo): reuse the gh CLI session token.
export GITHUB_PAT="$(gh auth token)"
# Option B.2 (least privilege): a fine-grained PAT scoped to the Grubify repo.
# export GITHUB_PAT="<your-fine-grained-PAT>"   # type it in the terminal, never commit it
Student/Resources/scenarios/scripts/sre-agent-config.sh apply --target connectors --name github-mcp \
  --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
Student/Resources/scenarios/scripts/sre-agent-config.sh verify --target connectors --name github-mcp \
  --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
unset GITHUB_PAT
```

> **Status (2026-06-12):** applied live to `contoso-sre-agent-dev` using
> `gh auth token` (`lpassaretta_microsoft`, scopes incl. `repo`). The bearer token
> is a point-in-time snapshot stored in the connector — if `gh` rotates its OAuth
> token, re-run the apply. For a predictable expiry and least privilege (only the
> `grubify` repo), use Option B.2 (fine-grained PAT) instead.

The official remote endpoint, HTTP transport, and `Authorization: Bearer <PAT>`
header are documented at https://github.com/github/github-mcp-server (Remote GitHub
MCP Server).

**Run it.** Trigger the same incident as S1 (`break-sample-food-app.sh`),
then in the SRE Agent portal chat ask:

> "For the current Grubify 5xx incident, analyze the API repository, identify the
> likely offending code path, and open a GitHub issue with your findings."

**Expected agent behavior.** The agent correlates the incident with the repo,
cites the suspect code, and (with GitHub connected) opens an issue/PR linking back
to the incident.

**Talk track.** "We've gone from 'the service is unhealthy' to 'here is the file,
here is the proposed fix, here is the issue assigned to the owning team' — without
a human switching tools."

**Restore.** Close any demo issue/PR created. Stop the load.

**Troubleshooting.** "GitHub tool not authorized" → the OAuth authorization (Option
A) is not complete, or the MCP connector (Option B) is not added.
...
```bash
Student/Resources/scenarios/scripts/sre-agent-config.sh verify --target scheduled-tasks \
...
```bash
Student/Resources/scenarios/scripts/sre-agent-config.sh verify --target knowledge-files \
  --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
```

### 12.5 KQL catalog

Reusable queries live in [kql-catalog.md](kql-catalog.md) and are runnable via
