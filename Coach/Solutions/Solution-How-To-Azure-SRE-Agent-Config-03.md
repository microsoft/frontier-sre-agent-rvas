...
> **Part of the Coach solution** — deploy first with [Solution 00](Solution-How-To-Deploy-All-00.md); see also the [Architecture (01)](Solution-Architecture-01.md) and [Demo Runbook (02)](Solution-Demo-Runbook-02.md). All commands run from `Coach/Solutions/`.

This guide explains how to use `Infra/scripts/sre-agent-config.sh` to validate, plan, apply, verify, and manage Azure SRE Agent configuration stored under `AZ-SRE-Agent-Configuration/`.
...
| `--file` | Select a single manifest or knowledge file by path. The script infers the target when possible. | `--file AZ-SRE-Agent-Configuration/skills/traffic-analytics-kql-analysis.yaml` |
...
4. Controlled fallback discovery for `AZ-SRE-Agent-Configuration/` and legacy `configuration/`.
...
| --- | --- | --- |
| `skills` | `AZ-SRE-Agent-Configuration/skills/` | `PUT /api/v2/extendedAgent/skills/{name}` |
| `subagents` | `AZ-SRE-Agent-Configuration/subagents/` | `PUT /api/v2/extendedAgent/agents/{name}` |
| `tools` | `AZ-SRE-Agent-Configuration/tools/` | Known preview-blocked in this tenant; script skips manifests marked `spec.deployment.status: api-preview-blocked` |
| `common-prompts` | `AZ-SRE-Agent-Configuration/common-prompts/` | `PUT /api/v2/extendedAgent/commonprompts/{name}` |
| `scheduled-tasks` | `AZ-SRE-Agent-Configuration/automations/scheduled-tasks/` | `PUT /api/v2/extendedAgent/scheduledtasks/{name}` |
| `incident-filters` | `AZ-SRE-Agent-Configuration/automations/incident-filters/` | `PUT /api/v2/extendedAgent/incidentFilters/{name}` |
| `incident-platforms` | `AZ-SRE-Agent-Configuration/incident-platforms/` | ARM PATCH on `properties.incidentManagementConfiguration` |
| `connectors` | `AZ-SRE-Agent-Configuration/connectors/` | `PUT /api/v2/extendedAgent/connectors/{name}` |
| `repos` | `AZ-SRE-Agent-Configuration/repos/` | `PUT /api/v2/repos/{name}` |
| `hooks` | `AZ-SRE-Agent-Configuration/hooks/` | `PUT /api/v2/extendedAgent/hooks/{name}` |
| `plugin-configs` | `AZ-SRE-Agent-Configuration/plugin-configs/` | `PUT /api/v2/extendedAgent/plugins/{name}` |
| `http-triggers` | `AZ-SRE-Agent-Configuration/automations/http-triggers/` | `POST /api/v1/httptriggers/create` |
| `plugin-marketplaces` | `AZ-SRE-Agent-Configuration/plugins/marketplaces/` | `POST /api/v2/plugins/marketplaces` |
| `plugin-installations` | `AZ-SRE-Agent-Configuration/plugins/installations/` | `POST /api/v2/plugins/installations` |
| `knowledge-files` | `AZ-SRE-Agent-Configuration/knowledge/files/` | `POST /api/v1/agentmemory/upload` |
...
Use full deployment only after reviewing every file under `AZ-SRE-Agent-Configuration/`, especially manifests that contain placeholders or secrets.
...
ERROR: Full desired-state apply completed with 2 failure(s):
  - connectors: .AZ-SRE-Agent-Configuration/connectors/<name>.yaml (exit 1)
  - incident-platforms: .AZ-SRE-Agent-Configuration/incident-platforms/<name>.yaml (exit 1)
```
...
```bash
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/skills/traffic-analytics-kql-analysis.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
```
...
```bash
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/subagents/network-traffic-analyst.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
```
...
```bash
./Infra/scripts/sre-agent-config.sh apply  --file AZ-SRE-Agent-Configuration/tools/my-tool.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```bash
./Infra/scripts/sre-agent-config.sh apply  --file AZ-SRE-Agent-Configuration/common-prompts/my-prompt.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```bash
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/automations/scheduled-tasks/flow-log-ingestion-freshness.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
```
...
```bash
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/automations/incident-filters/network-observability-review.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
```
...
./Infra/scripts/sre-agent-config.sh apply  --file AZ-SRE-Agent-Configuration/incident-platforms/pagerduty.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```bash
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/connectors/microsoft-learn-mcp.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
```
...
```bash
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/repos/grubify.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
```
...
```bash
./Infra/scripts/sre-agent-config.sh apply  --file AZ-SRE-Agent-Configuration/hooks/my-hook.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```bash
./Infra/scripts/sre-agent-config.sh apply  --file AZ-SRE-Agent-Configuration/plugin-configs/my-plugin-config.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```bash
./Infra/scripts/sre-agent-config.sh apply  --file AZ-SRE-Agent-Configuration/automations/http-triggers/my-trigger.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```bash
./Infra/scripts/sre-agent-config.sh apply  --file AZ-SRE-Agent-Configuration/plugins/marketplaces/my-marketplace.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```bash
./Infra/scripts/sre-agent-config.sh apply  --file AZ-SRE-Agent-Configuration/plugins/installations/my-installation.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```bash
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/knowledge/files/sample-food/http-500-errors.md --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
```
...
```text
AZ-SRE-Agent-Configuration/skills/example-diagnostics-skill.yaml
AZ-SRE-Agent-Configuration/skills/example-diagnostics-skill.md
```

Commands:

```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/skills/example-diagnostics-skill.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/skills/example-diagnostics-skill.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```text
AZ-SRE-Agent-Configuration/subagents/example-observability-subagent.yaml
```

Commands:

```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/subagents/example-observability-subagent.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/subagents/example-observability-subagent.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```text
AZ-SRE-Agent-Configuration/tools/example-arg-tool.yaml
```

Commands:

```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/tools/example-arg-tool.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/tools/example-arg-tool.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```text
AZ-SRE-Agent-Configuration/common-prompts/example-incident-summary.yaml
AZ-SRE-Agent-Configuration/common-prompts/example-incident-summary.md
```

Commands:

```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/common-prompts/example-incident-summary.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/common-prompts/example-incident-summary.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```text
AZ-SRE-Agent-Configuration/automations/scheduled-tasks/example-daily-health.yaml
```

Commands:

```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/automations/scheduled-tasks/example-daily-health.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/automations/scheduled-tasks/example-daily-health.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```text
AZ-SRE-Agent-Configuration/automations/incident-filters/example-sev2-filter.yaml
```

Commands:

```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/automations/incident-filters/example-sev2-filter.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/automations/incident-filters/example-sev2-filter.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```text
AZ-SRE-Agent-Configuration/incident-platforms/example-azure-monitor.yaml
```

Commands:

```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/incident-platforms/example-azure-monitor.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/incident-platforms/example-azure-monitor.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh verify --target incident-platforms --name azure-monitor --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
```
...
```text
AZ-SRE-Agent-Configuration/incident-platforms/example-pagerduty.yaml
```
...
```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/incident-platforms/example-pagerduty.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/incident-platforms/example-pagerduty.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh verify --target incident-platforms --name pagerduty --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
```
...
```text
AZ-SRE-Agent-Configuration/connectors/example-dynatrace-mcp.yaml
```
...
```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/connectors/example-dynatrace-mcp.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/connectors/example-dynatrace-mcp.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```text
AZ-SRE-Agent-Configuration/repos/example-repo.yaml
```

Commands:

```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/repos/example-repo.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/repos/example-repo.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```text
AZ-SRE-Agent-Configuration/hooks/example-stop-hook.yaml
```

Commands:

```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/hooks/example-stop-hook.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/hooks/example-stop-hook.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```text
AZ-SRE-Agent-Configuration/plugin-configs/example-plugin-config.yaml
```

Commands:

```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/plugin-configs/example-plugin-config.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/plugin-configs/example-plugin-config.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```text
AZ-SRE-Agent-Configuration/automations/http-triggers/example-maintenance-window.yaml
```

Commands:

```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/automations/http-triggers/example-maintenance-window.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/automations/http-triggers/example-maintenance-window.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```text
AZ-SRE-Agent-Configuration/plugins/marketplaces/example-marketplace.yaml
```

Commands:

```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/plugins/marketplaces/example-marketplace.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/plugins/marketplaces/example-marketplace.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```text
AZ-SRE-Agent-Configuration/plugins/installations/example-installation.yaml
```

Commands:

```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/plugins/installations/example-installation.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/plugins/installations/example-installation.yaml --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```text
AZ-SRE-Agent-Configuration/knowledge/files/example-runbook.md
```

Commands:

```bash
./Infra/scripts/sre-agent-config.sh plan --file AZ-SRE-Agent-Configuration/knowledge/files/example-runbook.md --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
./Infra/scripts/sre-agent-config.sh apply --file AZ-SRE-Agent-Configuration/knowledge/files/example-runbook.md --subscription "$SUB" --resource-group "$RG" --agent "$AGENT"
...
```bash
./Infra/scripts/sre-agent-config.sh validate --file AZ-SRE-Agent-Configuration/skills/example-diagnostics-skill.yaml
```
...
```bash
./Infra/scripts/sre-agent-config.sh validate --config /path/to/customer/AZ-SRE-Agent-Configuration
```
...
If you rename `AZ-SRE-Agent-Configuration/skills` to `AZ-SRE-Agent-Configuration/custom-skills`, full deployment no longer finds those skill manifests, `--target skills` no longer scans them, and `--file AZ-SRE-Agent-Configuration/custom-skills/name.yaml` can no longer infer the target automatically.
...
```text
AZ-SRE-Agent-Configuration/skills/customer-diagnostics.yaml
```

This file is not applied by full `apply` or `--target skills`:

```text
AZ-SRE-Agent-Configuration/skills/customer/customer-diagnostics.yaml
```

Full `validate` scans YAML files recursively under `AZ-SRE-Agent-Configuration/`, so a nested file can pass validation while still being ignored by `apply`. Keep deployable manifests directly in the documented target directories.
...
- A file under `AZ-SRE-Agent-Configuration/skills/` is treated as target `skills` and deployed to `/api/v2/extendedAgent/skills/{name}`.
- A file under `AZ-SRE-Agent-Configuration/repos/` is treated as target `repos` and deployed to `/api/v2/repos/{name}`.
...
Do not rename internal folders under `AZ-SRE-Agent-Configuration/` unless you also update `Infra/scripts/sre-agent-config.sh`, this guide, and every command example that depends on the target-to-folder mapping.
