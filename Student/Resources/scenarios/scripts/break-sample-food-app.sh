...
weather_code="$(curl -m 10 -s -o /dev/null -w "%{http_code}" "${api_url}/WeatherForecast" || echo "000")"
echo "Initial WeatherForecast API: HTTP ${weather_code}"

success_count=0
error_count=0
for request_number in $(seq 1 "${request_count}"); do
  http_code="$(curl -m 10 -s -o /dev/null -w "%{http_code}" \
...
final_weather_code="$(curl -m 10 -s -o /dev/null -w "%{http_code}" "${api_url}/WeatherForecast" || echo "000")"
food_items_code="$(curl -m 10 -s -o /dev/null -w "%{http_code}" "${api_url}/api/FoodItems" || echo "000")"
