...
The default behavior is read-only or an azd/configuration preview. Missions 00–02 preview staged azd changes and require `-Execute` to deploy them. Mission 00 runs `azd up`; Missions 01–02 run `azd provision`. Missions 03 and 04 are deliberate exceptions: customers run and inspect each command directly from the challenge page. The VM heartbeat action additionally supports `-Restore`.
...
| 05 | Inventory live connectors | None | Demonstrate configured versus proven connectivity |
